...
$RunningPath = Split-Path $mypath -Parent
Write-host $RunningPath
$locations = "$($RunningPath)\CNF\","$($RunningPath)\APP\","$($RunningPath)\CMP\"

foreach ($location in $locations) 
{

    $files = Get-childitem $location
    if ($location -like "*CMP*") { $ImportType = "Compliance" }
    if ($location -like "*CNF*") { $ImportType = "Configuration" }
    if ($location -like "*APP*") { $ImportType = "App Protection" }
    
    foreach ($file in $files) {
    
    $JSON_Data = gc $file.fullname
    ...
    $JSON_Convert = $JSON_Data | ConvertFrom-Json | Select-Object -Property * -ExcludeProperty id,createdDateTime,lastModifiedDateTime,version,supportsScopeTags
     ...
     $JSON_Output = $JSON_Convert | ConvertTo-Json -Depth 5
            ...
            write-host "Device $($ImportType) Policy $($JSON_COnvert.displayName) Found..." -ForegroundColor Green
            Write-Host "Adding $($ImportType) $($JSON_COnvert.displayName) " -ForegroundColor Yellow
            write-host 
            try {
            if ($location -like "*CMP*") { Add-DeviceCompliancePolicy -JSON $JSON_Output}
            if ($location -like "*CNF*") { Add-DeviceConfigurationPolicy -JSON $JSON_Output}
            if ($location -like "*APP*") { Add-ManagedAppPolicy -json $JSON_Output }
            } catch  {
            Write-Host "Error adding $($ImportType) $($JSON_COnvert.displayName)"
            }
